...
public class MucInfoDao extends BaseDao<Muc.MucItem> {
    ...
    public List<Muc.MucItem> selectAll() {
        ...
        try {
            db.beginTransaction();
            cursor = db
                .rawQuery("select * from " + DBHelper.TABLE_MUC_INFO + " order by " + DBHelper.ROLE
                    + " desc", null);
...
//            values.put(DBHelper.ROLE, mucItem.getPConfig().getRole().ordinal());
            ...
            if (mucItem.getPConfig().getRole() == null || mucItem.getPConfig().getRole() == Role.Member){
                if (!TextUtils.isEmpty(mucItem.getCreator()) && currentUserId.equalsIgnoreCase(mucItem.getCreator())) {
    ...
    public boolean insertMultiple(Context context, List<Muc.MucItem> mucItems,String currentUserId) {
...
//            //删成员
//            db.delete(DBHelper.TABLE_PRIVATE_MCU_USER, null, null);
            db.setTransactionSuccessful();
            if (result != -1) {
                return true;
            }
    ...
    public boolean updateMucInfo(String mucId, Muc.MucItem mucItem,String currentUserId) {
        if (!TextUtils.isEmpty(mucId)) {
            ...
            try {
                ...
                db.update(DBHelper.TABLE_MUC_INFO, values, String.format("%s=?", DBHelper.MUC_ID),
     ...
     */
    public void updateListMucInfo(List<Muc.MucItem> newMucItems, List<Muc.MucItem> oldMucItems,String currentUserId) {
            ...
            try {
                ...
                for (Muc.MucItem mucItem : newMucItems) {
                    ...
                    int result = db.update(DBHelper.TABLE_MUC_INFO, values,
                        String.format("%s=?", DBHelper.MUC_ID), new String[]{mucItem.getMucid()});
                    if (result > 0) {
                        continue;
                    } else {
                        db.insert(DBHelper.TABLE_MUC_INFO, null, values);
                    ...
                    //删成员
                    db.delete(DBHelper.TABLE_PRIVATE_MCU_USER,
                        String.format("%s=?", DBHelper.MUC_ID), new String[]{mucItem.getMucid()});
                    //增加群成员
