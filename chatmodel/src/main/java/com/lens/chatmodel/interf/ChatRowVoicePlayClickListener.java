...
public class ChatRowVoicePlayClickListener implements View.OnClickListener {
    ...
    public ChatRowVoicePlayClickListener(IChatRoomModel message, ImageView v,
        ImageView iv_read_status,
        RecyclerView.Adapter adapter, Context context) {
    ...
    private void getVoicePath(IChatRoomModel model) {
        ...
        if (!message.isIncoming()) {
            json = model.getUploadUrl();
        } else {
            json = model.getContent();
        }
        VoiceUploadEntity entity = VoiceUploadEntity.fromJson(json);
        if (entity == null) {
            return;
        }
        String uri = entity.getVoiceUrl();
        if (!message.isIncoming()) {
            ...
            .subscribe(new Consumer<String>() {
                ...
                public void accept(@NonNull final String path) throws Exception {
                    ...
                    try {
                        ...
                        isPlaying = true;
                        currentPlayListener = ChatRowVoicePlayClickListener.this;
                        mediaPlayer.start();
                        showAnimation();
                    } catch (Exception e) {
                        e.printStackTrace();
