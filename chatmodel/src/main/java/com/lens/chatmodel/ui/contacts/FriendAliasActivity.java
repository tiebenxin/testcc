...
public class FriendAliasActivity extends BaseUserInfoActivity {
    ...
    public static Intent newIntent(Context context, String alias, String userId, String group) {
        ...
        switch (type) {
            case CHANGE_ALIAS:
                ...
                if (savedInstanceState == null) {
                    fragment = FriendAliasFragment.newInstance(alias, userjid, group);
    ...
    protected void confirm() {
        resultName = fragment.getResult();
        switch (type) {
            case CHANGE_ALIAS:
                if (resultName.equalsIgnoreCase(alias)) {
                    backPressed();
                } else {
            ...
            if (message != null && message.response != null) {
                if (message.response.getCode() == Common.UPDATE_ROSTER_SUCCESS) {//更新成功
                    if (!TextUtils.isEmpty(userjid)) {
                        ProviderUser
                            .updateRosterRemarkName(ContextHelper.getContext(), userjid,
                                resultName);
                        setResult(RESULT_OK);
                        finish();
                    }
    ...
    public void backPressed() {
        if (fragment != null && fragment.hasChangeGroupName()) {
            setResult(RESULT_OK);
        }
        finish();
    }
