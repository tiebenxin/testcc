...
import com.lens.chatmodel.view.emoji.EmojiconPagerView.EaseEmojiconPagerViewListener;
import com.lens.chatmodel.view.emoji.EmojiconScrollTabBar.EaseScrollTabBarItemClickListener;
...
public class EmojiconMenu extends EmojiconMenuBase{
	...
	public void init(List<EmojiconGroupEntity> groupEntities){
	    ...
	    for(EmojiconGroupEntity groupEntity : groupEntities){
	        emojiconGroupList.add(groupEntity);//添加新的实例
    ...
    public void reset(List<EmojiconGroupEntity> groupEntities){
        if(groupEntities == null || groupEntities.size() == 0){
            return;
        }
