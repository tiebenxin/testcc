...
public class CircleMovementMethod extends BaseMovementMethod {
  ...
  public boolean onTouchEvent(TextView widget, Spannable buffer,
    ...
    if (action == MotionEvent.ACTION_DOWN) {
      ...
      int line = layout.getLineForVertical(y);//返回触摸点的行数
      int off = layout.getOffsetForHorizontal(line, x);//返回触摸点在该行上的横向偏移量
      ...
      if (mClickLinks.length > 0) {
        ...
        mBgSpan = new BackgroundColorSpan(ContextHelper.getColor(mClickableSpanBgClorId));
  ...
  public void cancel(TextView widget, Spannable buffer) {
    cancel = true;
