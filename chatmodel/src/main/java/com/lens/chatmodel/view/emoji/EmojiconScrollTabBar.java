...
public class EmojiconScrollTabBar extends RelativeLayout{
    ...
    private void init(Context context, AttributeSet attrs){
        ...
        LayoutInflater.from(context).inflate(R.layout.lens_widget_emojicon_tab_bar, this);
    ...
    public void addTab(int icon){
        View tabView = View.inflate(context, R.layout.lens_scroll_tab_item, null);
        ImageView imageView = (ImageView) tabView.findViewById(R.id.iv_icon);
        imageView.setImageResource(icon);
        LinearLayout.LayoutParams imgParams = new LinearLayout.LayoutParams((int) TDevice.dpToPixel(tabWidth), LayoutParams.MATCH_PARENT);
        ...
        if(position < childCount){
            scrollView.post(new Runnable() {
                ...
                public void run() {
                    ...
                    //滑动范围，取值可能有问题
                    int childX = tabContainer.getChildAt(position).getLeft();
