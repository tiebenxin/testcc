...
public abstract class AbstractSlideExpandableListAdapter extends WrapperListAdapterImpl {
  ...
  private final int yNavBarTop = ScreenUtil.getHeight() - ScreenUtil.dp_to_px(53);
  private final int hDataRow = ScreenUtil.dp_to_px(77);
  private final int hActionRow = ScreenUtil.dp_to_px(96);
  private final int STEP = ScreenUtil.dp_to_px(5);
  ...
  private void animateView(final View target, final int type) {
    ...
    if (params.bottomMargin < 0) {
      int[] coord = new int[2];
      target.getLocationInWindow(coord);
      int yExpandedBottom = coord[1] + hDataRow + hActionRow;
      if (yExpandedBottom > yNavBarTop) {
        final long start = System.currentTimeMillis();
        handler.postDelayed(new Runnable() {
          @Override
          public void run() {
            int[] coo = new int[2];
            target.getLocationInWindow(coo);
            if (coo[1] + hActionRow < yNavBarTop || System.currentTimeMillis() - start >= 500) {
              return;
            }
