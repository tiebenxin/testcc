...
public class LoggingInterceptor implements Interceptor {
  ...
  public Response intercept(Chain chain) throws IOException {
    ...
    Log.d(TAG,
        String.format(Locale.getDefault(), "Send Method : %s, Request Url : %s", method, url));
    ...
    //响应头
    Log.d(TAG, String.format(Locale.getDefault(), "Received response header is %s",
        response.headers().toString()));

    //从网络上获取数据时打印相关信息
    Log.d(TAG, "Received network response message is : " + response.networkResponse());

    //从缓存上获取数据时打印相关信息
    Log.d(TAG, "Received cache response message is : " + response.cacheResponse());
    ...
    Log.d(TAG,
        String.format(Locale.getDefault(), "Received response json string [%s]", bodyString));
