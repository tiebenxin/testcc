...
public abstract class RxSubscriberHelper<T> implements Observer<T> {
    ...
    protected void onShowMessage(ApiException apiException) {
        if (apiException != null && !TextUtils.isEmpty(apiException.message)) {
            Toast.makeText(LensCore.sContext, apiException.message, Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(LensCore.sContext, LensCore.sContext.getApplicationContext().getString(R.string.common_net_error_default), Toast.LENGTH_SHORT).show();
        }
    }
}
