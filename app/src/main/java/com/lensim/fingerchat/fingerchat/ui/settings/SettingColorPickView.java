...
public class SettingColorPickView extends View {
     ...
     **/
    private int[] colors = {
        Color.parseColor("#ff0000"), Color.parseColor("#ffffff"), Color.parseColor("#c3a407"),
        Color.parseColor("#fae268"), Color.parseColor("#68fa6f"),
        Color.parseColor("#057605"), Color.parseColor("#50e2e4"), Color.parseColor("#1981ca"),
        Color.parseColor("#0d3fbe"), Color.parseColor("#ca69fc"),
        Color.parseColor("#f590e3"), Color.parseColor("#c47293"), Color.parseColor("#ed4fc6")
    };
    /**
     * 颜色块画笔
     **/
    ...
    private OnColorPickListenr onColorPickListenr;

    public SettingColorPickView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    private void calColorBar() {
        ...
        if (rectFs == null && width != 0 && height != 0) {
            ...
            //知道
            RectF rectF1 = new RectF(startPoint.x + this.colorBarHeight / 2, startPoint.y,
                startPoint.x + colorBarwidth, startPoint.y + this.colorBarHeight);
            rectFs.add(rectF1);
            int length = colors.length - 1;
            for (int i = 1; i < length; i++) {
                RectF rectF = new RectF(startPoint.x + i * colorBarwidth, startPoint.y,
                    startPoint.x + (i + 1) * colorBarwidth, startPoint.y + this.colorBarHeight);
                rectFs.add(rectF);
            }

            RectF rectF2 = new RectF(startPoint.x + length * colorBarwidth, startPoint.y,
                startPoint.x + colors.length * colorBarwidth - this.colorBarHeight / 2,
                startPoint.y + this.colorBarHeight);
            rectFs.add(rectF2);

            startRect = new RectF(startPoint.x, startPoint.y, startPoint.x + this.colorBarHeight,
                startPoint.y + this.colorBarHeight);
            endRect = new RectF(startPoint.x + colors.length * colorBarwidth - this.colorBarHeight,
                startPoint.y, startPoint.x + colors.length * colorBarwidth,
                startPoint.y + this.colorBarHeight);
...
//            blockindex = 1;
            ...
            block = new RectF(blockCenter.x - blockWidth / 2, blockCenter.y - blockheight / 2,
                blockCenter.x + blockWidth / 2, blockCenter.y + blockheight / 2);
        }
    }
...
//        canvas.drawRect(300,400,500,450,paint);
        ...
        if (drawText) {
            ...
            float textCenterVerticalBaselineY =
                circlePoint.y - fm.descent + (fm.descent - fm.ascent) / 2;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                //根据索引位置，让滑块回到中间
                startAnim();
                if (onColorPickListenr != null) {
                    onColorPickListenr.onColorPick(colors[blockindex]);
                }
                break;
    ...
    public void setOnColorPickListenr(OnColorPickListenr onColorPickListenr) {
        this.onColorPickListenr = onColorPickListenr;
    }
