...
import com.lensim.fingerchat.fingerchat.ui.me.collection.DisplayImageActivity;
import com.lensim.fingerchat.fingerchat.ui.me.collection.NoteActivity;
...
public class RichTextEditor extends ScrollView {
    ...
    private OnClickListener closeListener; // 图片右上角红叉按钮监听器
    private OnClickListener mapListener; // 地图监听器
    private OnClickListener showPictureListener; // 图片查看大图监听器
    ...
    public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // 3. 图片叉掉处理
        closeListener = new OnClickListener() {
            @Override
        ...
        // 3. 查看大图片
        showPictureListener = new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (v instanceof NoteImageView) {
                    String imgUrl = ((NoteImageView) v).getAbsolutePath();
                    if (!StringUtils.isEmpty(imgUrl)) {
                        DisplayImageActivity.openActivity((Activity) getContext(), imgUrl);
                    }
                }
            }
        };
        ...
        mapListener = new OnClickListener() {
            ...
            public void onClick(View v) {
                if (v instanceof NoteImageView) {
                    String url = ((NoteImageView) v).getAbsolutePath();
                    try {
                        String[] path = url.split(NoteStringUtils.SPLIT_AND);
                        if (path[0].indexOf(NoteStringUtils.SPLIT_COMER) != -1) {
                            String[] info = path[0].split(NoteStringUtils.SPLIT_COMER);
                            double latitude = Double.parseDouble(info[1]);
                            double longtitude = Double.parseDouble(info[2]);
                            MapInfoEntity addressInfo = new MapInfoEntity(info[0], info[3],
                                latitude, longtitude);
                            MapPickerActivity.openActivity((Activity) getContext(),
                                NoteActivity.REQUEST_NOTE_PICK_LOCATION, addressInfo);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                        MapPickerActivity
                            .openActivity((Activity) getContext(),
                                NoteActivity.REQUEST_NOTE_PICK_LOCATION, null);
                    }
                }
            }
        };
        ...
        EditText firstEdit = createEditText(null, dip2px(context, EDIT_PADDING));
...
//    }
    ...
    public EditText createEditText(String hint, int paddingTop) {
        EditText editText = (EditText) inflater.inflate(R.layout.note_item_edittext, null);
     ...
     * 插入一张图片
     */
    public void insertImage(String imgUrl, int type) {
        ...
        if (lastEditStr.length() == 0 || editStr1.length() == 0) {
            ...
            addImageViewAtIndex(lastEditIndex, imgUrl, type);
        } else {
            ...
            addImageViewAtIndex(lastEditIndex + 1, imgUrl, type);
    ...
    public void addImageViewAtIndex(final int index, String url, int type) {
        ...
        if (type == NoteImageView.TYPE_IMG) {
            ...
            imageView.setOnClickListener(showPictureListener);
        } else if (type == NoteImageView.TYPE_MAP) {
            Glide.with(getContext()).load(NoteStringUtils.divideImagePath(url, false))
                .into(imageView);
            imageView.setOnClickListener(mapListener);
        }
        ...
        LinearLayout.LayoutParams firstEditParam = new LinearLayout.LayoutParams(
            LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
        //editNormalPadding = dip2px(EDIT_PADDING);
        EditText endEdit = createEditText("", EDIT_PADDING);
        allLayout.addView(endEdit, firstEditParam);
    }
    ...
    public List<EditData> buildEditData() {
        ...
        for (int index = 0; index < num; index++) {
            ...
            if (itemView instanceof EditText) {
                ...
                NoteImageView item = (NoteImageView) itemView.findViewById(R.id.edit_imageView);
