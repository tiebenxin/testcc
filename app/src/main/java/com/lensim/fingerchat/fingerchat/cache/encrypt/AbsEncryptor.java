...
/**
 * Created by LY309313 on 2017/5/18.
 */
...
public abstract class AbsEncryptor {
   ...
   * 创建或获取已缓存的盐值。
   *
   * @param pref 用于保存配置的{@link SharedPreferences}
   */
  public static byte[] getOrCreateSalt(int saltLen, @Nullable SharedPreferences pref,
      @NonNull String key) {
    // String saltHex = pref != null ? pref.getString(key, null) : CoreConfig.getInstance().getString(key, null);
    String saltHex = SPHelper.getString(key, null);
    ...
    if (salt == null) {
      ...
      SPHelper.saveValue(key, saltHex);
//            if (pref != null) {
...
//            }
   ...
   * @param pref 用于保存配置的{@link SharedPreferences}
   * @param saltName 用于保存盐的 hex 字符串的 key， 为 null 时将使用 AES+password+keyBitLen 拼接成的字符串的 MD5值。
   */
  public static byte[] getOrDeriveKey(String password, int keyBitLen,
   ...
   */
  public static byte[] deriveKey(String password, int keyBitLen)
   ...
   * 建议优先使用{@link #getOrDeriveKey(String, int, Integer, SharedPreferences, String)}
   *
   * @param keyBitLen 可选值如下：{@link #KEY_BITS_LENGTH_128}、{@link #KEY_BITS_LENGTH_192}、{@link
   * #KEY_BITS_LENGTH_256}(为 null 时的默认值)
   * @param iterationCount 用于创建{@link PBEKeySpec}，为 null 时默认值是{@link #ITERATION_COUNT}
   */
  public static byte[] deriveKey(String password, int keyBitLen, @Nullable byte[] secureSalt,
      @Nullable Integer iterationCount) throws NoSuchAlgorithmException, InvalidKeySpecException {
    int saltLen = keyBitLen / 8;
  ...
  @Deprecated
  public static byte[] deriveKeyDeprecated(String password, String algorithm, int keyBitLen)
  ...
  @Deprecated
  public static byte[] deriveKeyDeprecated(String password, String algorithm, int keyBitLen,
      @Nullable Charset charset) throws NoSuchProviderException, NoSuchAlgorithmException {
    SecureRandom secureRandom = SecureRandom
        .getInstance(DEPREACATED_SECURE_ALGORITHM_SHA1PRNG, DEPREACATED_SECURE_PROVIDER_CRYPTO);
   ...
   * <p> Stripped-down version of the SHA1PRNG provided by the Crypto provider. <p> The Crypto
   * provider that offers this functionality was deprecated on Android. <p> Use this class only to
   * retrieve encrypted data that couldn't be retrieved otherwise.
   */
  private static class InsecureSHA1PRNGKeyDerivator {
     ...
     *
     * @params arrW - integer array; arrW.length >= (BYTES_OFFSET+6); <BR> only first
     * (BYTES_OFFSET+6) words are used
     */
     ...
     * a calling method is responsible for such checks.
     *
     * @params intArray  - int array containing bytes to which to append; intArray.length >=
     * (BYTES_OFFSET+6)
     * @params byteInput - array of bytes to use for the update
     * @params from      - the offset to start in the "byteInput" array
     * @params to        - a number of the last byte in the input array to use, that is, for first
     * byte "to"==0, for last byte "to"==input.length-1
     */
