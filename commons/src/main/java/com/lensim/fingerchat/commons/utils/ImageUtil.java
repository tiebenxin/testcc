...
public class ImageUtil {
  ...
  private static Bitmap createWaterMaskBitmap(Bitmap src, Bitmap watermark, Rect mSrcRect, Rect mDestRect) {
    if (src == null) {
      return null;
    }
    int width = src.getWidth();
    ...
    canvas.drawBitmap(src, 0, 0, null);
    //在画布上绘制水印图片
    canvas.drawBitmap(watermark, mSrcRect, mDestRect, null);
    // 保存
    canvas.save(Canvas.ALL_SAVE_FLAG);
    // 存储
    canvas.restore();
   ...
   * 设置水印图片到左下角
   */
  public static Bitmap createWaterMaskLeftBottom(Bitmap src, Bitmap watermark, Rect mSrcRect, Rect mDestRect) {
    return createWaterMaskBitmap(src, watermark, mSrcRect, mDestRect);
  }
